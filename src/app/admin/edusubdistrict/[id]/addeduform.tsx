...
export function AddEduSubform() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (token) {
      const response = await axios.post(`${apiURL}/adminEdit/modifyEduSubDistrict`, formdata, {
        headers: {
    ...
    <Dialog>
      <DialogTrigger asChild>
        <div className="flex items-center justify-start gap-2 my-4 cursor-pointer text-primary  float-right">
          <Plus />
          <span className="text-base">Add EduSubDistrict</span>
        </div>
      </DialogTrigger>
      <DialogContent className="max-w-4xl overflow-y-scroll max-h-[98%]">
        <DialogHeader>
          <DialogTitle>Add EduSubDistrict</DialogTitle>
