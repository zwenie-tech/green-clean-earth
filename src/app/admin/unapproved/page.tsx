...
const AdminGrid = () => {
  ...
  const onRowClicked = (event: RowClickedEvent) => {
    ...
    router.push(`unapproved/${id}`);
  };

  const handleExportToExcel = async () => {
    try {
      const response = await axios.post(`${apiURL}/admin/adminUnApprovedList`, {
        "isExcel": true
      }, {
        headers: {
  ...
  useEffect(() => {
    async function fetchdata() {
      if (token && Object.keys(filterdata).length === 0) {
        ...
        const response = await axios.post(`${apiURL}/admin/adminUnApprovedList?page=${currentPage}&limit=${itemsPerPage}`, {}, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
  ...
  const fetchFilteredCoordName = async (value: string) => {
    if (token) {
      const response = await axios.post(
        `${apiURL}/admin/adminUnApprovedList`,
...
setTotalcount("0");
  ...
  useEffect(() => {
    async function fetchFilterData() {
      ...
      const response = await axios.post(
        `${apiURL}/admin/adminUnApprovedList?page=${currentPage}&limit=${itemsPerPage}`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${token}`,
