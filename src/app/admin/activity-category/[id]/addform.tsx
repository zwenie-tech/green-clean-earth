...
export function AddActCatForm() {
  ...
  async function onSubmit() {
    ...
    if (token) {
      try {
        const response = await axios.post(`${apiURL}/adminEdit/modifyActivityCategory`, formdata, {
          headers: {
    ...
    <Dialog>
      <DialogTrigger asChild>
        <div className="flex items-center justify-start gap-2 my-4 cursor-pointer text-primary float-right">
          <Plus />
          <span className="text-base">Add Activity Category</span>
        </div>
      </DialogTrigger>
      <DialogContent className="max-w-4xl overflow-y-scroll max-h-[98%]">
        <DialogHeader>
          <DialogTitle>Add Activity Category</DialogTitle>
