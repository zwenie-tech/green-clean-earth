...
import { AddActCatForm } from "./[id]/addform";
...
const AdminGrid = () => {
  ...
  const [columnDefs, setColumnDefs] = useState<ColDef[]>([
    { field: "activity_category_id", headerName: "Id" },
    { field: "activity_category", headerName: "Category" },
    { field: "activity_category_created_on", headerName: "Created on" },
  ...
  const onRowClicked = (event: RowClickedEvent) => {
    ...
    const id = event.data.activity_category_id;
  ...
  useEffect(() => {
    async function fetchdata() {
      if (token) {
        ...
        const response = await axios.get(`${apiURL}/admin/getActivityCategory`, {
          headers: {
    ...
    const formattedEvents = response.data.count.map((event: { activity_category_created_on: string | number | Date; }) => {
      const date = new Date(event.activity_category_created_on);
      ...
      return {
          ...event,
          activity_category_created_on: `${day}-${month}-${year}`
      };});

    localStorage.setItem("actcat", JSON.stringify(formattedEvents));
