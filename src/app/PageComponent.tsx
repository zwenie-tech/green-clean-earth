      ...
      <div className="flex justify-center items-center space-x-1">
        {getPaginationRange().map((page, index) => {
          ...
          return (
            <button
              key={page}
              className={`text-xl py-1 px-3 sm:text-sm sm:px-2 sm:py-1 rounded-md ${page === currentPage
            ...
            >
              ...
              <form onSubmit={(e) => { 
                ...
                const pageValue = form.page.value; 
                handlePageChange(Number(pageValue)); 
            }}>
              <input 
                type="text" 
                name="page" 
                ...
                pattern="[0-9]*" 
                className='w-auto h-7 min-w-10 max-w-5 text-center text-sm border border-[#3C6E1F] outline-none focus:ring-1 focus:ring-green-600 focus:border-green-600 rounded appearance-none' 
                placeholder={page+''} 
                onChange={(e) => {
                  const value = e.target.value;

                  if (/^\d*$/.test(value)) { // Only allows digits (no decimals or letters)
